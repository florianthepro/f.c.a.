...
<body>
    ...
    <script>
        ...
        function selectFirmwareForFlash(fw) {
            const btn = document.querySelector(`.firmware-option:contains(${fw.toUpperCase()})`);
            if (!btn) {
                document.querySelectorAll('.firmware-option').forEach(el => {
                    if (el.textContent === fw.toUpperCase()) {
                        selectFirmware(fw, el);
                    }
                });
            } else {
                selectFirmware(fw, btn);
            }
...
if (!isset($sources[$firmware])) exit;

try {
    $ch = curl_init();
    curl_setopt_array($ch, [
        CURLOPT_URL => $sources[$firmware],
    ...
    if (!$data || !isset($data['assets'])) throw new Exception('No assets found');
    ...
    if (!$downloadUrl) throw new Exception('No binary found');
    ...
    file_put_contents($fwDir . '/.error', $e->getMessage());
    @unlink($stateFile);
}
?>
PHP;
...
function getDownloadProgress($firmware) {
    ...
    // Prüfe ob Download abgeschlossen
